...
if (isset($_FILES["photo"]["name"]) && isset($_POST["gallery"])){
	...
	$row = mysqli_fetch_row($query);
	if($row[0] > 19){
		header("location: ../message.php?msg=The demo system allows only 20 pictures total");
        exit();	
	}
	$gallery = preg_replace('#[^a-z 0-9,]#i', '', $_POST["gallery"]);
    $description = preg_replace('#[^a-z 0-9,]#i', '', $_POST["description"]);
	...
	$sql = "INSERT INTO photos(user, gallery, filename, uploaddate, description) VALUES ('$log_username','$gallery','$db_file_name',now(), '$description')";
	$query = mysqli_query($db_connect, $sql);
	mysqli_close($db_connect);
